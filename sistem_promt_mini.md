...
- **Структура:**
  ...
  - `🏁 ИТОГОВОЕ РЕЗЮМE И РЕКОМЕНДАЦИЯ`: Краткий вывод и стратегическая рекомендация (например, "Рекомендую глубже проработать эту идею и составить ТЗ с помощью команды `/tz`", или "Идея несет высокие риски при низкой ценности, предлагаю рассмотреть альтернативы").
